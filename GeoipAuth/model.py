...
class User(BaseModel):
    ...
    def set_phone_number(self, phone: str) -> bool:
        if db.session.execute(
            db.select(User).filter_by(PhoneNumber=phone)
        ).scalar_one_or_none():
            return False
        else:
...
class Ticket(BaseModel):
    ...
    UserID = Column(
        Integer,
        ForeignKey(BaseModel.SetTableName("users") + ".id"),
        nullable=False,
        unique=False,
    )
    ...
    def set_user_id(self, user: User) -> None:
        self.UserID = user.id
...
class AnswerTicket(BaseModel):
    ...
    AdminID = Column(
        Integer,
        ForeignKey(BaseModel.SetTableName("admins") + ".id"),
        nullable=False,
        unique=False,
    )
    TicketID = Column(
        Integer,
        ForeignKey(BaseModel.SetTableName("tickets") + ".id"),
        nullable=True,
        unique=False,
    )
